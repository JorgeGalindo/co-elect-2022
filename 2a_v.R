...
raw_df <- read_csv("https://raw.githubusercontent.com/nelsonamayad/Elecciones-presidenciales-2022/main/Encuestas%202022/encuestas_2022.csv") %>%
  ...
  mutate(
    #Meto el último dato de GAD3 a mano - al ser un tracking es necesario
    fecha=case_when(
      encuestadora=="GAD3"  ~ as.Date("2022-06-04"),
      TRUE ~ fecha
    ),
    gustavo_petro=case_when(
      encuestadora=="GAD3"  ~ 46.3,
      
      TRUE ~ gustavo_petro
    ),
    rodolfo_hernandez=case_when(
      encuestadora=="GAD3"  ~ 47.9,
      TRUE ~ rodolfo_hernandez
    ),
    blanco=case_when(
      encuestadora=="GAD3"  ~ 5.4,
      TRUE ~ blanco
    ),
...
write.csv(promedio_raw_df,"modelo_2av/promedio_norm_2av.csv")
